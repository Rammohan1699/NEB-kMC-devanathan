...
def relax_one_site(
    ...
    if args.relax == "h-only":
        ...
        raise ValueError("--relax must be h-only or local")
...
def main() -> None:
    ...
    ap.add_argument("--relax", choices=["h-only", "local"], default="h-only", help="For lammps mode: relax only H or H + nearby Fe")
