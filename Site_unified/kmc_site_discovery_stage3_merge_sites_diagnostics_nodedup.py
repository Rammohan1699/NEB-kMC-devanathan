...
def read_lammps_box(path: str) -> Box:
    ...
    return Box(bounds[0, 0], bounds[0, 1], bounds[1, 0], bounds[1, 1], bounds[2, 0], bounds[2, 1])
...
def _row_value(row: Dict[str, str], keys: Iterable[str], default: str = "") -> str:
...
def _row_int(row: Dict[str, str], keys: Iterable[str], default: int = -1) -> int:
    text = _row_value(row, keys, "")
    if text == "":
        return default
    try:
...
def build_site_metadata(rows: List[Dict[str, str]], labels: List[str]) -> Dict[str, np.ndarray]:
    grain_ids: List[int] = []
    support_grains: List[str] = []
    n_support_grains: List[int] = []
    interface_class: List[str] = []
    source_site_id: List[str] = []

    for row, label in zip(rows, labels):
        grain_id = _row_int(row, ["grain_id", "nearest_grain_id"], -1)
        grains = _row_value(row, ["support_grains", "grains"], "")
        n_grains = _row_int(row, ["n_support_grains", "num_support_grains"], -1)
        if n_grains < 0:
            n_grains = len([x for x in grains.split(";") if x != ""]) if grains else (1 if grain_id >= 0 else 0)
        iface = _row_value(row, ["interface_class", "interface_type"], "")
        if not iface:
            iface = "BULK_GRAIN" if label == "BULK_TETRA" else "GB_RELAXED"

        grain_ids.append(grain_id)
        support_grains.append(grains)
        n_support_grains.append(n_grains)
        interface_class.append(iface)
        source_site_id.append(_row_value(row, ["site_id", "candidate_id", "id"], ""))

    return {
        "grain_id": np.asarray(grain_ids, dtype=int),
        "support_grains": np.asarray(support_grains, dtype=object),
        "n_support_grains": np.asarray(n_support_grains, dtype=int),
        "interface_class": np.asarray(interface_class, dtype=object),
        "source_site_id": np.asarray(source_site_id, dtype=object),
    }


def write_sites_csv(path: str, pos: np.ndarray, types: np.ndarray, labels: List[str], metadata: Dict[str, np.ndarray]) -> None:
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "site_id", "x", "y", "z", "site_type", "site_label", "grain_id",
            "support_grains", "n_support_grains", "interface_class", "source_site_id",
        ])
        for i, (r, t, lab) in enumerate(zip(pos, types, labels), start=1):
            j = i - 1
            writer.writerow([
                i, f"{r[0]:.10f}", f"{r[1]:.10f}", f"{r[2]:.10f}", int(t), lab,
                int(metadata["grain_id"][j]),
                str(metadata["support_grains"][j]),
                int(metadata["n_support_grains"][j]),
                str(metadata["interface_class"][j]),
                str(metadata["source_site_id"][j]),
            ])
...
def main() -> None:
    ...
    bulk_pos, bulk_rows = read_sites_csv(args.bulk_sites)
    gb_pos, gb_rows = read_sites_csv(args.gb_sites)
    ...
    clean_bulk_rows = [row for row, remove in zip(bulk_rows, remove_bulk) if not remove]
    ...
    source_rows = clean_bulk_rows + gb_rows
    metadata = build_site_metadata(source_rows, labels)
    ...
    write_sites_csv(f"{args.out_prefix}_unified_sites.csv", unified_pos, unified_types, labels, metadata)
    ...
    np.savez_compressed(
        ...
        grain_id=metadata["grain_id"],
        support_grains=metadata["support_grains"],
        n_support_grains=metadata["n_support_grains"],
        interface_class=metadata["interface_class"],
        source_site_id=metadata["source_site_id"],
    )
