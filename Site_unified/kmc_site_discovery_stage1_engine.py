...
def read_lammps_box(path: Path) -> np.ndarray:
    return read_lammps_box_bounds(path)
...
def main() -> None:
    ...
    ap.add_argument("--fe-types", default=None, help="Comma-separated Fe atom types. Use for polycrystal files that encode grains as atom types.")
    ...
    if args.fe_types:
        cmd_region.extend(["--fe-types", str(args.fe_types)])
    else:
        cmd_region.extend(["--fe-type", str(args.fe_type)])
    ...
    if args.fe_types:
        cmd_gb.extend(["--fe-types", str(args.fe_types)])
