...
def read_lammps_atomic_data(path: str) -> LammpsData:
    ...
    for i, line in enumerate(lines):
        if line.strip().startswith("Atoms"):
            ...
            break
    if start is None:
        raise ValueError("Could not find Atoms section in LAMMPS data file.")
    ...
    for line in lines[start:]:
        ...
        if n_atoms is not None and len(rows) >= n_atoms:
            break
    if not rows:
        raise ValueError("No atoms parsed from Atoms section.")
    arr = np.array(rows, dtype=float)
    return LammpsData(arr[:, 0].astype(int), arr[:, 1].astype(int), arr[:, 2:5], box)
...
def load_region_masks(mask_path: str, n_atoms: int) -> Tuple[np.ndarray, np.ndarray]:
    m = np.load(mask_path)
    if "region_code_all" in m:
        ...
        fe_global = m["fe_indices_global"].astype(int)
        if fe_global.min() == 1:
...
def main():
    ...
    region, grain = load_region_masks(args.masks, len(pos))

    fe_mask = np.ones(len(pos), dtype=bool)  # current Fe-only structures; keep robust for type filters later.
    trans_mask = region == 2
    gb_mask = region == 3
    seed_mask = fe_mask & (trans_mask | gb_mask)
    ...
    cl = PeriodicCellList(pos, data.box, cutoff=max(2.0, min(search_cut, 4.0)))
    ...
    # Build many small local Voronoi diagrams instead of one global Voronoi
    # tessellation. This is cheaper and avoids filling deep bulk regions that
    # are already handled by the analytical tetra mapper.
    for count, seed in enumerate(seed_indices, start=1):
        ...
        for v in vor.vertices:
            if not np.all(np.isfinite(v)):
                continue
            # Keep only vertices close to the current seed atom. Vertices from a
            # local Voronoi patch can lie far outside the physically relevant
            # seed neighborhood.
    ...
    # Merge duplicate vertices produced by overlapping local patches. The class
    # label is preserved as GB_CORE when any merged member had GB support.
    ...
    with open(csv_path, "w") as f:
        f.write("site_id,x,y,z,candidate_class,multiplicity\n")
        for i, (p, c, m) in enumerate(zip(merged, mclass, mult), start=1):
            cname = "GB_CORE" if c == 1 else "TRANSITION"
            f.write(f"{i},{p[0]:.10f},{p[1]:.10f},{p[2]:.10f},{cname},{int(m)}\n")
    xyz_path = f"{out}.xyz"
    lmp_path = f"{out}.lmp"
    npz_path = f"{out}.npz"
    write_extended_xyz(xyz_path, merged, mclass, data.box)
    write_lammps_sites(lmp_path, merged, mclass, data.box)
    np.savez_compressed(npz_path, positions=merged, candidate_class=mclass, multiplicity=mult, box=data.box)
