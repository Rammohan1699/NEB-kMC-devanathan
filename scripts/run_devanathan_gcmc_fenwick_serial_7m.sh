...
# Run the fresh GCMC/Fenwick Devanathan calculation as independent serial
# processes. Each process advances 500,000 KMC steps, then the next process
# restarts from its final checkpoint and preloads its merged barrier cache.
...
INITIAL_CACHE="${INITIAL_CACHE:-runs/devanathan_generated_90x20x20_sink0p8_accumulated_cache_through_7000000.pkl}"
...
CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/devanathan_gcmc_fenwick_mu_m1p75_7m_serial_$(date +%Y%m%d_%H%M%S)}"
RESUME="${RESUME:-0}"
DRY_RUN="${DRY_RUN:-0}"
...
if [[ "$DRY_RUN" != "1" && ! -s "$INITIAL_CACHE" ]]; then
...
echo "GCMC + Fenwick segmented Devanathan campaign"
...
gcmc_source="$SCRIPT_DIR/run_devanathan_gcmc_fenwick_7m.sh"
base_source="$SCRIPT_DIR/run_devanathan_kmc.sh"
if [[ ! -s "$gcmc_source" || ! -s "$base_source" ]]; then
  echo "ERROR: Missing launcher file(s) in $SCRIPT_DIR:" >&2
  [[ -s "$gcmc_source" ]] || echo "  $gcmc_source" >&2
  [[ -s "$base_source" ]] || echo "  $base_source" >&2
  echo "Restage the Hyperion package with the complete scripts directory." >&2
  exit 1
fi
if [[ ! -e "$launcher_snapshot/run_devanathan_gcmc_fenwick_7m.sh" ]]; then
  cp -p "$gcmc_source" "$launcher_snapshot/"
fi
if [[ ! -e "$launcher_snapshot/run_devanathan_kmc.sh" ]]; then
  cp -p "$base_source" "$launcher_snapshot/"
fi
bash -n \
  "$launcher_snapshot/run_devanathan_gcmc_fenwick_7m.sh" \
  "$launcher_snapshot/run_devanathan_kmc.sh"
gcmc_launcher="$launcher_snapshot/run_devanathan_gcmc_fenwick_7m.sh"
