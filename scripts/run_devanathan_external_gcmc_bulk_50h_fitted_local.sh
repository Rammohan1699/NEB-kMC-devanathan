...
# Local external-map Devanathan production using the bulk-GCMC calibration
# from the 50-H Sigma5 bulk-grain sweep. This starts from production GCMC
# initialization, then runs DEVANATHAN_SOURCE_MODE=gcmc_bulk.
...
export RUN_ROOT="${RUN_ROOT:-runs/devanathan_external_gcmc_bulk_${region_tag}_${target_tag}H_mu_${mu_tag}_local_$(date +%Y%m%d_%H%M%S)}"

echo "External-map fitted 50-H bulk-GCMC local run"
...
bash "$SCRIPT_DIR/run_devanathan_external_gcmc_bulk_kmc.sh"
